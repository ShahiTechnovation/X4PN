...
interface WithdrawModalBlockchainProps {
  open: boolean;
...
export function WithdrawModalBlockchain({
  open,
  onOpenChange,
  usdcBalance,
  x4pnBalance,
  onWithdraw,
}: WithdrawModalBlockchainProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Withdraw (Blockchain)</DialogTitle>
          <DialogDescription>
            Withdraw your earnings directly to your wallet through the blockchain
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            <Tabs value={token} onValueChange={(v) => setToken(v as "usdc" | "x4pn")}>
              <TabsList className="grid w-full grid-cols-2">
                ...
                <TabsTrigger value="x4pn" className="gap-2">
